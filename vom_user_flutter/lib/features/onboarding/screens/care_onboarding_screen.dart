...
// 각 단계 파일들 import
import 'step_00_splash.dart';
import 'step_01_welcome.dart';
import 'step_02_loading.dart';
import 'step_03_card_intro.dart';
import 'step_04_name_input.dart';
import 'step_05_phone_check.dart';
import 'step_06_otp_input.dart';
import 'step_07_verification_success.dart';
import 'step_07_b_child_info.dart';
import 'step_08_interests.dart';
import 'step_09_processing.dart';
import 'step_10_completion.dart';
...
class _CareOnboardingScreenState extends State<CareOnboardingScreen> {
  ...
  // Step 7B 완료 콜백 (아이 나이 정보 저장)
  void _onChildAgeSelected(String age) {
    setState(() {
      _childAge = age;
    });
    _nextPage(); // Step 7B -> Step 8로 이동
  }
      ...
      child: Scaffold(
        ...
        body: PageView(
          ...
          children: [
            ...
            // [Step 1] 환영
            Step01Welcome(onNext: _nextPage, onBack: _prevPage),

            // [Step 2] 로딩
            Step02Loading(onLoadingComplete: _nextPage, onBack: _prevPage),

            // [Step 3] 카드 소개 (이름 없음)
            Step03CardIntro(userName: _userName, onNext: _nextPage, onBack: _prevPage),
            ...
            Step07VerificationSuccess(
              userName: _userName,
              onNext: _nextPage, // 다음 단계(아이 나이 선택)로
              onBack: _prevPage,
            ),

            // [Step 7B] 아이 나이 선택
            Step07BChildInfo(
              onNext: _onChildAgeSelected,
              onBack: _prevPage,
            ),
